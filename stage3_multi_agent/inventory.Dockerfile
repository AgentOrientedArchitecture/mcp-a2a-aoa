# Enhanced Inventory Agent Dockerfile with Telemetry
...
exec python stage3_multi_agent/agents/inventory_agent.py\n\
...
HEALTHCHECK --interval=30s --timeout=10s --start-period=5s --retries=3 \
    CMD curl -f http://localhost:8002/.well-known/agent-card.json || exit 1
